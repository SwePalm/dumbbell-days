...
function showHelp(name){const [title,body]=tips[name]||['A deliberate, controlled rep','Choose a weight that lets you keep the movement smooth and pain-free.'];const image=formImages[name];const visual=image?`<img src="assets/${image}" alt="Illustration of ${name}">`:`${icon()}`;const modal=document.querySelector('#modal');modal.className='modal open';modal.innerHTML=`<div class="sheet"><button class="sheet-close" aria-label="Close">×</button><div class="diagram ${image?'illustrated':''}">${visual}<span>FORM CUE</span></div><p>EXERCISE GUIDE</p><h2>${name}</h2><h3>${title}</h3><div class="rule"></div><p class="tip">${body}</p><div class="safety">If you feel sharp pain, stop. Adjust range, load, or choose an alternative.</div></div>`;modal.onclick=e=>{if(e.target===modal||e.target.closest('.sheet-close')){modal.className='modal';modal.innerHTML=''}}}
